...
app.use("/ping", (req, res) => res.send("cSharp is up!"))
...
app.post("/", middleWare, (req, res) => {
  try {
    ...
    const exeutableName = `${name}.exe`

    fs.writeFile(fileName, data, (err) => {
      if (err) return res.status(500).send(err)

      exec(`csc ${fileName}`, options, (error, stdout, stderr) => {
    ...
    console.log("cSharp => ", err)
    res.sendStatus(500)
  }
})

app.listen(port, () => {
  console.log(`cSharp microservice listening on ${port}`)
})